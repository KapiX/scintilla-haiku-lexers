...
static const char* LexerName = "Bash";
...
Sci_Position SCI_METHOD LexBash::PropertySet(const char *key, const char *val) {
...
Sci_Position SCI_METHOD LexBash::WordListSet(int n, const char *wl) {
...
void SCI_METHOD LexBash::Lex(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument *pAccess) {
...
void SCI_METHOD LexBash::Fold(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument *pAccess) {
	if(!options.fold)
		return;
...
LexerFactoryFunction EXT_LEXER_DECL GetLexerFactory(unsigned int index) {
	if (index == 0)
		return LexBash::LexerFactoryBash;
	else
		return 0;
}
